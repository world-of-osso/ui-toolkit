...
pub fn parse_const_declarations(source: &str) -> Vec<(String, String)> {
    ...
    while let Some(pos) = rest.find("const ") {
        ...
        if let Some(v) = value { consts.push((name.to_string(), v)); }
    }
    consts
}

fn extract_const_value(rhs: &str) -> Option<String> {
    if rhs.starts_with('"') { return extract_string_literal(rhs); }
    let paren_pos = rhs.find('(')?;
    let after_paren = rhs[paren_pos + 1..].trim_start();
    if after_paren.starts_with('"') { extract_string_literal(after_paren) } else { None }
}
...
fn is_ws_or_comma(b: u8) -> bool { matches!(b, b' ' | b'\n' | b'\r' | b'\t' | b',') }

fn skip_whitespace(input: &str, pos: usize) -> usize {
    let bytes = input.as_bytes();
    let mut p = pos;
    while p < input.len() && (bytes[p] == b' ' || bytes[p] == b'\n' || bytes[p] == b'\r' || bytes[p] == b'\t') { p += 1; }
    p
}
...
fn read_ident(input: &str, pos: usize) -> (usize, &str) {
    ...
    let start = pos;
    let mut p = pos;
    while p < input.len() && (bytes[p].is_ascii_alphanumeric() || bytes[p] == b'_') { p += 1; }
    (p, &input[start..p])
}
...
fn parse_children(input: &str, consts: &[(String, String)]) -> Vec<WidgetChild> {
    ...
    let bytes = input.as_bytes();
    while pos < input.len() {
        while pos < input.len() && is_ws_or_comma(bytes[pos]) { pos += 1; }
        ...
        if pos < input.len() && bytes[pos] == b'{' {
            let Some(block) = extract_braced_block(&input[pos..]) else { pos += 1; continue };
...
fn skip_else_block(input: &str, pos: usize) -> usize {
    ...
    if !rest.starts_with("else") { return pos; }
...
fn skip_to_brace(s: &str) -> &str {
    if let Some(p) = s.find('{') { &s[p..] } else { s }
}

fn dispatch_element(tag: &str, block: &str, consts: &[(String, String)]) -> WidgetChild {
    if tag == "anchor" { parse_anchor_element(block, consts) }
    else { parse_element(tag, block, consts) }
}
...
fn parse_element(tag: &str, block: &str, consts: &[(String, String)]) -> WidgetChild {
    ...
    if let Some(name_attr) = widget.attrs.iter().find(|a| a.effective_name() == "name") {
        if let AttrValue::Static(ref s) = name_attr.value { widget.name = Some(s.clone()); }
    }
...
fn collect_attrs(block: &str, consts: &[(String, String)]) -> Vec<Attr> {
    ...
    while pos < block.len() {
        while pos < block.len() && is_ws_or_comma(bytes[pos]) { pos += 1; }
...
fn parse_attr_value(s: &str, consts: &[(String, String)]) -> (Option<String>, usize) {
    ...
    if let Some((val, n)) = try_parse_string_literal(trimmed) { return (Some(val), leading + n); }
    if let Some((val, n)) = try_parse_number(trimmed) { return (Some(val), leading + n); }
    if let Some((val, n)) = try_parse_bool(trimmed) { return (Some(val), leading + n); }
...
fn try_parse_bool(s: &str) -> Option<(String, usize)> {
    ...
    if s.starts_with("true") && s[4..].starts_with(not_ident) { return Some(("true".to_string(), 4)); }
    if s.starts_with("false") && s[5..].starts_with(not_ident) { return Some(("false".to_string(), 5)); }
    None
}

fn try_parse_known_ident(ident: &str, ident_len: usize, s: &str, consts: &[(String, String)]) -> Option<(String, usize)> {
    ...
    if let Some(v) = ident.strip_prefix("FrameStrata::") { return Some((v.to_string(), ident_len)); }
    if let Some(v) = ident.strip_prefix("DrawLayer::") { return Some((v.to_string(), ident_len)); }
    if let Some(v) = ident.strip_prefix("GameFont::") { return Some((v.to_string(), ident_len)); }
    if let Some(v) = ident.strip_prefix("AnchorPoint::") { return Some((map_anchor_point(v).to_string(), ident_len)); }
    if let Some(v) = ident.strip_prefix("JustifyH::") { return Some((map_justify_h(v).to_string(), ident_len)); }
...
fn read_qualified_ident(s: &str) -> (String, usize) {
    ...
    loop {
        let seg_start = pos;
        while pos < s.len() && (bytes[pos].is_ascii_alphanumeric() || bytes[pos] == b'_') { pos += 1; }
        if pos == seg_start { break; }
        if pos + 1 < s.len() && bytes[pos] == b':' && bytes[pos + 1] == b':' { pos += 2; } else { break; }
    }
    (s[..pos].to_string(), pos)
}
...
fn extract_paren_content(s: &str) -> Option<&str> {
    ...
    let mut depth = 0;
    for (i, c) in s.char_indices() {
        match c { '(' => depth += 1, ')' => { depth -= 1; if depth == 0 { return Some(&s[1..i]); } } _ => {} }
    }
    None
}
...
fn map_justify_h(v: &str) -> &str {
    match v { "Left" => "LEFT", "Right" => "RIGHT", "Center" => "CENTER", other => other }
}
...
fn skip_value(s: &str) -> usize {
    ...
    while pos < s.len() {
        match bytes[pos] {
            b'"' => { let n = scan_string_literal_len(&s[pos..]); pos += if n == 0 { 1 } else { n }; continue; }
            b'(' | b'[' | b'{' => depth += 1,
            b')' | b']' | b'}' => { if depth == 0 { return pos; } depth -= 1; }
...
mod tests {
    ...
    fn find_attr<'a>(attrs: &'a [Attr], name: &str) -> Option<&'a str> {
        attrs.iter().find(|a| a.effective_name() == name).map(|a| a.value_str())
    }
    ...
    fn parse_simple_element() {
        ...
        let WidgetChild::Widget(ref w) = templates[0].defs[0] else { panic!("expected Widget") };
    ...
    fn parse_nested_children() {
        ...
        let templates = parse_rsx_blocks(source, "test.rs");
        assert_eq!(templates.len(), 1);
        let WidgetChild::Widget(ref outer) = templates[0].defs[0] else { panic!() };
        assert_eq!(outer.children.len(), 1);
        let WidgetChild::Widget(ref inner) = outer.children[0] else { panic!() };
    ...
    fn parse_anchor_pseudo_element() {
        ...
        let templates = parse_rsx_blocks(source, "test.rs");
        let WidgetChild::Widget(ref frame) = templates[0].defs[0] else { panic!() };
